...
private_key= b'\xdc\x90\xac\xe0?\xcac+\n\x19\n\x97\xd5v\xa1\x9d\xdd\x08\x86\xd9y\xd6\xee\xbf\x7f\x83\x8e\xad\x11&UE'
private_key2 = b'\xc0\xac\x18\xd5\x89\xca\xd6k\xfc={%\x89\x19\xc9\x9c\xfc\xaf\x07\xc1\xb00\x0b\xadz@4\xfb\xe3C\xc1\xb9'

signing_key = SigningKey.generate()
sk1 = SigningKey.from_string(private_key, curve=SECP256k1)
sk2 = SigningKey.from_string(private_key2, curve=SECP256k1)


keys = {'8090': {'pub': sk1.get_verifying_key().to_string(), 'priv': sk1}, 
        '8091': {'pub': sk2.get_verifying_key().to_string(), 'priv': sk2}}
...
class ChatClient(Frame):
    ...
    def handleClientMessages(self, clientsoc, clientaddr):
        while 1:
            try:
                ...
                if self.vefifyng(msg, signature, clientsoc):
                    msg = '(trusted) ' + msg
                else:
                    msg += '(untrusted info) ' + msg
    ...
    def vefifyng(self, msg, signature, clientsoc):
